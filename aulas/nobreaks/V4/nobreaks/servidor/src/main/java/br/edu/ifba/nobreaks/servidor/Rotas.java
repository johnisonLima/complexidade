...
public class Rotas {
    ...
    @POST
    @Path("{id}/{temperatura}/{carga}/{oscilacoes}")
    public Response gravarLeitura(@PathParam("id") String idNoBreak, @PathParam("temperatura") int temperatura, @PathParam("carga") int cargaBateria, @PathParam("oscilacoes") int oscilacoes) {
        NoBreak noBreak = new NoBreak(idNoBreak, "único");
        Leitura leitura = new Leitura(temperatura, cargaBateria, oscilacoes);
